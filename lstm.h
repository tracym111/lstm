...
int word_dim;
  ...
  const std::vector<Param*> GetParams() const override
  {
    std::vector<Param*> params{weight_h_i_,weight_x_i_,weight_h_f_,weight_x_f_,weight_h_o_,weight_x_o_,weight_h_g_,weight_x_g_};
    return params;
  }
...
class UnrollLayer : public LSTMLayer , public singa::ConnectionLayer {
 public:
  ~UnrollLayer();
...
class LossLayer : public LSTMLayer
{
 public:
  ~LossLayer();
  void Setup(const LayerProto& conf, const vector<Layer*>& srclayers) override;
